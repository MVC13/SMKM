...
def SMKM(D,k,start):   
    ...
    #####################
    n=D.shape[0];d=D.shape[1]
    i=0;STOP=False;DC3=0;ITU=0;
    while STOP==False: 
        i+=1       
        if i==1:
            if start==0:
        ...
        if Jin03>=Jin0:
            ...
            print i,Jin03,DC3,ITU
            Jin0=Jin03;Desc=np.zeros(k);tt=1
            ...
            for j in range(k):
                ...
                if len(X)>1:
                    ...
                    k_meansINT.fit(X);DC3=k_meansINT.n_iter_*2*len(X)+DC3+len(X);
            ...
            C03=np.vstack((C03,CC[kk][0]));C03=np.vstack((C03,CC[kk][1]))
            lU=np.hstack((lU,l0C[kk]));lU=np.hstack((lU,l1C[kk]))       
            Z = cdist(C03,C03,'sqeuclidean');tt=0
            Z0=copy.deepcopy(Z)            
            for j in range(k):
                for m in range(j,k+1):
                    t=(lU[j]*lU[m])/(lU[j]+lU[m])
                    #print 't:',t
                    Z0[j][m]=Z[j][m]*t;Z0[m][j]=0;  
            Z0[k-1][k]=0          
